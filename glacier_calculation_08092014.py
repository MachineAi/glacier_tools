...
#2014-09-08: v0.1.7 Adaption after Reviews for Gurla Mandhata
...
class MainInterface:
    ...
    def testPlots(self, pNpGlacier_, demNameNew_, demNameOld_, name_, ela_):
        ...
        plt.axvline(np.mean(pNpGlacier[:,3]), color='k', linestyle='dashed', linewidth=1) #draw a vertical line from the bottom to the top of the y axis
        plt.axvline(np.median(pNpGlacier[:,3]), color='g', linestyle='dashed', linewidth=1) #
        ...
        print ("Polynomal fitting for "+str(demNameNew_)+" minus "+str(demNameOld_)+" for '" + str(name_) + "':")
        print pPoly1
        ...
        print "Linear regression for "+str(demNameNew_)+" minus "+str(demNameOld_)+" for '" + str(name_) + "': y = " + str(round(slope,4)) + " * x +" + str(round(intercept,4)) + "; r_value, p_value, std_err:", rValue1, pValue, stdErr
        ...
        plt.plot(pNpGlacier[:,2], pNpGlacier[:,3], 'b.', ela_, 0, 'ko', pX, pPoly1(pX), 'k-', pX, y1, 'k--') #x1, y1, x2, y2 #plt.plot(t, t, 'r--', t, t**2, 'bs', t, t**3, 'g^')
    ...
    def gurlaPlots(self, pNpGlacier1_, pNpGlacier2_, demName1_, demName2_, name_, ela_):
        ...
        polyCoeffs = np.polyfit(pNpGlacier[:,2], pNpGlacier[:,3], deg=3) #Fit the data with a polynomial and get the polynomal coefficients
        pPoly1 = np.poly1d(polyCoeffs, variable='z') # construct the polynomial as Object
        print ("Polynomal fitting for "+str(demName1_)+" for '" + str(name_) + "':")
        print pPoly1
        
        #Scipy.stats.linregress for linear regression
        slope, intercept, rValue1, pValue, stdErr = stats.linregress(pNpGlacier[:,2], pNpGlacier[:,3])
        y1 = slope * pX + intercept
        print "Linear regression for "+str(demName1_)+" for '" + str(name_) + "': y = " + str(round(slope,4)) + " * x +" + str(round(intercept,4)) + "; r_value, p_value, std_err:", rValue1, pValue, stdErr
        ...
        print ("Polynomal fitting for "+str(demName2_)+" for '" + str(name_) + "':")
        print pPoly2
        
        #Scipy.stats.linregress for linear regression
        slope, intercept, rValue2, pValue, stdErr = stats.linregress(pNpGlacier[:,2], pNpGlacier[:,3])
        y2 = slope * pX + intercept
        print "Linear regression for "+str(demName2_)+" for '" + str(name_) + "': y = " + str(round(slope,4)) + " * x +" + str(round(intercept,4)) + "; r_value, p_value, std_err:", rValue2, pValue, stdErr
        
        #Plot
        plt.figure() 
        #plt.axhline(0, color='k', linestyle='solid', linewidth=1) #Zero line
        #plt.axvline(ela_, color='k', linestyle='solid', linewidth=1) #ELA line
        plt.plot(pNpGlacier[:,2], pNpGlacier[:,3], 'r.', ela_, 0, 'ko', pX, pPoly2(pX), 'k-', pX, y2, 'k--') #x1, y1, x2, y2 #plt.plot(t, t, 'r--', t, t**2, 'bs', t, t**3, 'g^')
        plt.xlabel('SRTM elevation (m a.s.l.)') #Z 
...
def main():
    ...
    try:
        ...
        if study == "halji":
            ...
            ela = 5660 #Halji ELA for glacier G081388E30289N (Shi, 2008) : 5660m
            
            #pNpNoGlacierTest = pInterfaceMain.testDtmAccuracy()
            #pInterfaceMain.testPlots(pNpNoGlacierTest, "input_DEM", "SRTM", "stable terrain", ela) #pNpGlacier_, demNameNew_, demNameOld_, name_, ela_
            
            glacierNamesList = ["G081470E30264N", "G081437E30281N", "G081393E30265N"] #Gletscher ID Unterschiede GLIMS zu RGI, G081388E30289N in Glims ist G081470E30264N in RGI
        ...
        if study == "mustag":
            ...
            #elasMustagDictOrig =  {"allglacier": "5200", "kekesayi": "4900", "G075233E38272N": "4800", "G075175E38297N": "4920", "G075153E38307N": "4940", "G075101E38308N": "5120", "kematulejia": "5940", \
            ...
            #Mustag Ata following kaeaebetal2012, SRTM penetration as mean of (HK+KK+JK)/3 regions: glacier 2.1+-0.4m; firn/snow: 4.3+-0.6m; clean ice: 1.5+-0.6m
            srtmPenetration = [-4.3, -1.5, 0.6] # penetrationDepthAcc, penetrationDepthAbl, srtmPenetrationError --> consider math sign!
        ...
        if study == "gurla":
            ...
            #Mean of all ELAs of Shi(2008) in study area: 6024.25 = ~6025
